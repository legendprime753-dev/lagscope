...
public final class LagController {
    ...
    public void start() {
        ...
        task = Bukkit.getScheduler().runTaskTimer(plugin, this::tick, 40L, 20L * interval);
    }
    ...
    private void tick() {
        ...
        int chunkEntTrigger = plugin.getConfig().getInt("auto.max-entities-per-chunk-trigger", 280);

        boolean lagging = m.tps1 < tpsTrigger || (m.mspt > 0 && m.mspt > msptTrigger);

        ModeSettings s = ModeSettings.fromConfig(plugin, mode);
        if (!lagging) {
            for (ChunkSnapshot cs : m.topChunks) {
                if (cs.entities >= chunkEntTrigger) {
                    lagging = true;
                    break;
                }
            }
        ...
        Map<String, Integer> worldItems = new HashMap<>();
        for (World w : Bukkit.getWorlds()) {
            int items = 0;
            for (Entity e : w.getEntities()) {
                if (e instanceof org.bukkit.entity.Item) items++;
            }
            worldItems.put(w.getName(), items);
        }

        for (World w : Bukkit.getWorlds()) {
            int items = worldItems.getOrDefault(w.getName(), 0);
            if (s.clearItemThresholdPerWorld > 0 && items >= s.clearItemThresholdPerWorld) {
        ...
        for (ChunkSnapshot cs : m.topChunks) {
            if (s.chunkClearEntityThreshold > 0 && cs.entities >= s.chunkClearEntityThreshold) {
                ...
                if (w == null) continue;
                Chunk chunk = w.getChunkAt(cs.key.x, cs.key.z);
                cleaner.clearChunk(chunk, s.aggressive);
                continue;
            }
    ...
    public ModeSettings currentSettings() {
        return ModeSettings.fromConfig(plugin, mode);
    }
}
