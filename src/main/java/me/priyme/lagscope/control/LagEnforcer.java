...
public final class LagEnforcer implements Listener {
    ...
    public void onCreatureSpawn(CreatureSpawnEvent event) {
        ...
        if (e instanceof Villager) {
            if (s.maxVillagersPerChunk > 0 && countTypeInChunk(chunk, EntityType.VILLAGER) >= s.maxVillagersPerChunk) {
        ...
        if (e.getType().isAlive()) {
            if (e.getType().isSpawnable()) {
                if (e.getType().isAlive() && e.getType().isSpawnable()) {
                    if (isMonster(e.getType())) {
                        if (s.maxMonstersPerChunk > 0 && countLivingInChunk(chunk, true) >= s.maxMonstersPerChunk) {
                            event.setCancelled(true);
                        }
                    } else {
                        if (s.maxAnimalsPerChunk > 0 && countLivingInChunk(chunk, false) >= s.maxAnimalsPerChunk) {
                            event.setCancelled(true);
                        }
                    }
                }
    ...
    public void onEntitySpawn(EntitySpawnEvent event) {
        ...
        if (e instanceof Item) {
            if (s.maxItemsPerChunk > 0 && countClassInChunk(chunk, Item.class) >= s.maxItemsPerChunk) {
                event.setCancelled(true);
            }
            return;
        }
        if (e instanceof ExperienceOrb) {
            if (s.maxXpOrbsPerChunk > 0 && countClassInChunk(chunk, ExperienceOrb.class) >= s.maxXpOrbsPerChunk) {
                event.setCancelled(true);
            }
            return;
        }
        if (e instanceof Projectile) {
            if (s.maxProjectilesPerChunk > 0 && countClassInChunk(chunk, Projectile.class) >= s.maxProjectilesPerChunk) {
                event.setCancelled(true);
            }
            return;
        }
        if (e instanceof TNTPrimed) {
            if (s.maxPrimedTntPerChunk > 0 && countClassInChunk(chunk, TNTPrimed.class) >= s.maxPrimedTntPerChunk) {
                event.setCancelled(true);
            }
        }
    }
    ...
    private static int countTypeInChunk(Chunk chunk, EntityType type) {
        int c = 0;
    ...
    private static int countLivingInChunk(Chunk chunk, boolean monsters) {
        int c = 0;
        for (Entity e : chunk.getEntities()) {
    ...
    private static <T> int countClassInChunk(Chunk chunk, Class<T> cls) {
        int c = 0;
        for (Entity e : chunk.getEntities()) {
            if (cls.isInstance(e)) c++;
        }
        return c;
    }
}
