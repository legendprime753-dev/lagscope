...
public final class MobAIOptimizerModule extends AbstractModule {
    ...
    private boolean nearPlayer(LivingEntity e, int dist) {
        for (Player p : e.getWorld().getPlayers()) {
            if (!p.isOnline()) continue;
            if (p.getLocation().distanceSquared(e.getLocation()) <= (double) dist * dist) return true;
        }
        return false;
    }
